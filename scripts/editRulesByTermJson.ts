...
async function editRulesByTerm() {
  ...
  try {
    const [allVerbsDataStr, rulesByTermDataStr, modelsDataStr] =
      await Promise.all([
    ...
    const allVerbsData: { [key: string]: VerbData } =
      JSON.parse(allVerbsDataStr);
    ...
    console.log(
      "Iniciando a busca por verbos correspondentes a cada terminação...",
    );
    ...
    for (let index = 0; index < totalKeys; index++) {
      ...
      if (
        specificMainKey &&
        ((Array.isArray(specificMainKey) &&
          !specificMainKey.includes(mainKey)) ||
      ...
      for (const subKey of subKeys) {
        ...
        for (let i = 0; i < filteredVerbs.length; i += batchSize) {
          ...
          const verbPropsPromises = batch.map(async (verb) => {
            ...
            if (props) {
              const termEntrie = props.termEntrie ?? "";
        ...
        subKeyData.type =
          subKeyData.abundance1 &&
          Object.entries(subKeyData.abundance1).length > 0
            ? [...uniqueClasses, 4]
    ...
    const totalVerbs = Object.values(rulesByTermData)
      .flatMap((mainKeyData) =>
        Object.values(mainKeyData).map(
          (subKeyData) => subKeyData.verbs?.total || 0,
        ),
      )
    ...
    console.log(
      "Verificando a ocorrência de verbos em mais de uma terminação...",
    );
...
async function compareAndLogMissingVerbs(
  ...
  console.log(
    "Comparando verbos de allVerbs.json com verbos em rulesByTerm.json...",
  );
  ...
  const missingVerbs = allVerbsArray.filter(
    (verb) => !entryOfArray.includes(verb),
  );
  ...
  if (missingVerbs.length > 0 && entryOfArray.length !== allVerbsArray.length) {
    ...
    console.log(
      "- todos os verbos de allVerbsData estão presentes nos entries.",
    );
  } else {
...
function extractVerbsEntries(rulesByTermData: RulesByTermData): {
  [key: string]: number[];
} {
    ...
    for (const subKey in mainKeyData) {
      ...
      if (subKeyData.verbs?.entries) {
        Object.entries(subKeyData.verbs.entries).forEach(([verb, models]) => {
          ...
          allEntries[verb] = Array.from(
            new Set([...allEntries[verb], ...models]),
          );
        });
      }
    }
