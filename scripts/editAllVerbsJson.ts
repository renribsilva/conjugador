...
async function ediAllVerbsJson() {
  ...
  try {
    ...
    if (!fs.existsSync(ptBRPath)) {
      ...
      await pullLibreOfficeWords(
        "https://raw.githubusercontent.com/LibreOffice/dictionaries/refs/heads/master/pt_BR/pt_BR.dic",
        ptBRPath,
      );
    } else {
    ...
    const normNewWords = newWords
      .map((verb) => nw(verb))
      .filter((verb) => verb !== "");
    const existingWordsSet = new Set(ptBRWords.map((verb) => nw(verb)));
    const filteredNewWords = normNewWords.filter(
      (verb) => !existingWordsSet.has(verb),
    );
    const notAddedWords = normNewWords.filter((verb) =>
      existingWordsSet.has(verb),
    );
    ...
    const allVerbsSet = updatedWords.filter(
      (word) =>
    ...
    console.log(
      `- quantidade de vocábulos terminados em 'ar', 'er', 'ir' e 'por': ${v1}`,
    );
    ...
    const duplicates = Array.from(normNonVerbs).filter((verb) =>
      normNewVerbs.has(verb),
    );

    if (duplicates.length > 0) {
      console.log(
        "- palavras duplicadas entre nonVerb e newVerbs:",
        duplicates,
      );
    } else {
      console.log(
        "- nenhuma palavra duplicada encontrada entre nonVerb e newVerbs",
      );
    }
    ...
    const finalVerbs = uniqueVerbs.map(
      (normVerb) => allVerbs[normalizedVerbs.indexOf(normVerb)],
    );
    // .filter(verb => /crer$/.test(verb));

    console.log(
      `- quantidade de verbos após remoção de duplicados: ${finalVerbs.length}`,
    );
    ...
    const processVerbsAsync = async (
      ...
      const processBatch = async (batch: string[], startIndex: number) => {
        ...
        const batchPromises = batch.map(async (verb) => {
          ...
          if (go) {
            try {
              ...
              let verbProps = cache.get(input);
              if (!verbProps) {
                verbProps = (await conjugateVerb(input, allVerbsJson))
                  .propOfVerb;
                cache.set(input, verbProps);
              }

              if (verbProps && typeof verbProps === "object") {
                const matchedTermination = verbProps.termination;
                ...
                if (
                  verbProps.model &&
                  Object.keys(verbProps.model).length > 0
                ) {
                  acc[input].model = verbProps.model;
                }
              }
            } catch (error) {
        ...
        const progress = Math.floor(
          ((startIndex + batch.length) / finalVerbs.length) * 100,
        );
        const elapsedTime = Date.now() - startTime;
        const remainingTime =
          ((elapsedTime / (startIndex + batch.length)) *
            (finalVerbs.length - (startIndex + batch.length))) /
          1000;
    ...
    const porCount = Object.keys(J).filter(
      (key) => key.endsWith("por") || key.endsWith("pôr"),
    ).length;
