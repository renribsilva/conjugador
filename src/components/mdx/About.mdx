...
na lista de palavras previamente processada.
Essa lista fica armazenada em cachê, utilizando o espaço de armazenamento
do próprio navegador, o que permite ao site funcionar mesmo sem conexão com a internet. 
Assim, nos bastidores (_server side rendering_), o radical da palavra pesquisada
é extraído, a fim de compor as flexões de acordo com as regras de conjugação
pré-definidas, gerando todas as formas verbais correspondentes a cada modo, tempo e flexão. 
Em seguida, essas formas conjugadas são organizadas 
e exibidas em uma tabela (_client side rendering_). Nesse processo, nenhum tipo 
de dado é coletado ou armazenado em nuvem: todo o processamento é feito no próprio
navegador.

### Da coleta de dados solicitada

Então, quer dizer que nenhum dado é coletado nessa giribosca? Pera lá! 
Sim, mas só se você quiser. A rigor, os únicos dados que 
interessam a este conjugador são aqueles que podem ajudar a corrigir 
erros ou expandir a lista de verbos. Assim, quando necessário, esta aplicação
pede solicitação para armazenar algum dado. E como isso funciona? Com botões! 
...
[^3]: Link para o DOI: [https://doi.org/10.11606/T.8.2007.tde-04102007-143455](https://doi.org/10.11606/T.8.2007.tde-04102007-143455)
