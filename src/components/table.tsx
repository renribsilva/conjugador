...
import Data from '../json/file.json'; // Ajuste o caminho para o seu arquivo JSON
import { nw } from '../lib/normalizeVerb';

interface ConjugationForms {
  [key: string]: string; // Por exemplo: { p1: "eu ..." }
}

interface Conjugations {
  pa: ConjugationForms;
  gd: ConjugationForms;
  pr_ind: ConjugationForms; // Presente do Indicativo
  pt1_ind: ConjugationForms; // Pretérito Imperfeito
  pt2_ind: ConjugationForms; // Pretérito Perfeito
  pt3_ind: ConjugationForms; // Pretérito Mais-que-perfeito
  ft1_ind: ConjugationForms; // Futuro do Presente
  ft2_ind: ConjugationForms; // Futuro do Pretérito
  pr_sub: ConjugationForms; // Presente do Subjuntivo
  pt_sub: ConjugationForms; // Pretérito Imperfeito do Subjuntivo
  fut_sub: ConjugationForms; // Futuro do Subjuntivo
  im1: ConjugationForms; // Imperativo Afirmativo
  im2: ConjugationForms; // Imperativo Negativo
  inf: ConjugationForms; // Infinitivo
}

interface VerbEntry {
  verb: string;
  conjugations: Conjugations; // Alterado para o novo formato
}
...
// Declara que Data é do tipo VerbEntry
const verbData: VerbEntry = Data as VerbEntry;


export default function Table({ verb }: TableProps) {
  const [conjugations, setConjugations] = useState<Conjugations | null>(null);

  useEffect(() => {
    const normalizedInputVerb = nw(verb);

    // Verifica se o verbo solicitado é o mesmo que o da entrada
    if (nw(verbData.verb) === normalizedInputVerb) {
      setConjugations(verbData.conjugations);
    }
  }, [verb]);

  // Se as conjugações ainda não estiverem disponíveis, não renderiza nada
  if (!conjugations) {
    return null; // Ou você pode retornar um loader aqui se desejar
  }
